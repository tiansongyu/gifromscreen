...
messages! {
    ...
    WatermarkTitle => ("watermark-title", [], "Raster watermark", "图片水印"),
    WatermarkFormatsHint => ("watermark-formats-hint", [], "PNG, JPEG, BMP, or WebP · decoded off the UI thread · maximum 4096²/64 MiB", "支持 PNG、JPEG、BMP 和 WebP；后台解码，上限为 4096² 像素 / 64 MiB。"),
    WatermarkImagePath => ("watermark-image-path", [], "Image path", "图片路径"),
    WatermarkSourceSizeHint => ("watermark-source-size-hint", [], "W/H = 0 keeps source dimensions", "宽度或高度为 0 时，该维度沿用原图尺寸。"),
    WatermarkImageOpacity => ("watermark-image-opacity", [], "Image opacity", "图片不透明度"),
    WatermarkDecoding => ("watermark-decoding", [], "Decoding watermark…", "正在解码水印…"),
    WatermarkAdd => ("watermark-add", [], "Decode and add watermark", "解码并添加水印"),
    WatermarkSelectFrames => ("watermark-select-frames", [], "Select at least one frame before adding a watermark.", "请先选择至少一帧，再添加水印。"),
    WatermarkNameRequired => ("watermark-name-required", [], "Watermark name is required.", "请输入水印名称。"),
    WatermarkOpacityRequired => ("watermark-opacity-required", [], "Watermark opacity must be greater than zero.", "水印不透明度必须大于零。"),
    WatermarkImageTrackOpacityRequired => ("watermark-image-track-opacity-required", [], "Watermark image and track opacity must be greater than zero.", "水印图片和轨道的不透明度都必须大于零。"),
    WatermarkTargetChanged => ("watermark-target-changed", [], "The project or selected frames changed while decoding. Select the intended frames and retry.", "解码期间项目或选中帧已改变。请重新选择目标帧后重试。"),
    WatermarkInvalidDisplaySize => ("watermark-invalid-display-size", ["error"], "Invalid watermark display size: {error}", "水印显示尺寸无效：{error}"),
    WatermarkBackgroundDecoding => ("watermark-background-decoding", [], "Decoding watermark in the background…", "正在后台解码水印…"),
    WatermarkStartFailed => ("watermark-start-failed", ["error"], "Could not decode watermark: {error}", "无法解码水印：{error}"),
    WatermarkProjectClosed => ("watermark-project-closed", [], "The editor project was closed while decoding. Open the intended project and retry.", "解码期间编辑器项目已关闭。请打开目标项目后重试。"),
    WatermarkTargetLost => ("watermark-target-lost", [], "The watermark authoring target was lost; retry.", "水印的编辑目标已丢失，请重试。"),
    WatermarkAdded => ("watermark-added", ["width", "height", "path"], "Added {width}×{height} watermark {path} to the selected frame span.", "已将 {width}×{height} 水印 {path} 添加到选中帧范围。"),
    WatermarkCommitFailed => ("watermark-commit-failed", ["error"], "Watermark decoded but could not be added: {error}. Adjust the form and retry.", "水印已解码，但无法添加：{error}。请调整表单后重试。"),
    WatermarkDecodeFailed => ("watermark-decode-failed", ["error"], "Could not decode watermark: {error}. Adjust the form and retry.", "无法解码水印：{error}。请调整表单后重试。"),
    WatermarkNoResult => ("watermark-no-result", [], "Watermark decoder finished without a result; retry safely.", "水印解码器已结束，但未返回结果；可以安全重试。"),
    WatermarkFileNotFound => ("watermark-file-not-found", ["path"], "Watermark image does not exist: {path}", "水印图片不存在：{path}"),
    WatermarkNotFile => ("watermark-not-file", ["path"], "Watermark input is not a regular file: {path}", "水印输入不是普通文件：{path}"),
    WatermarkInvalidExtension => ("watermark-invalid-extension", ["path"], "Watermark must end in PNG, JPG/JPEG, BMP, or WebP: {path}", "水印文件扩展名必须为 PNG、JPG/JPEG、BMP 或 WebP：{path}"),
    TextCaptionHint => ("text-caption-hint", [], "Add a caption…", "输入要添加的文字…"),
    TextFont => ("text-font", [], "Font", "字体"),
    TextFontSize => ("text-font-size", [], "Font size", "字号"),
    TextAlignment => ("text-alignment", [], "Alignment", "对齐方式"),
    TextAlignLeft => ("text-align-left", [], "Left", "左对齐"),
    TextAlignCenter => ("text-align-center", [], "Center", "居中"),
    TextAlignRight => ("text-align-right", [], "Right", "右对齐"),
    TextColor => ("text-color", [], "Text color", "文字颜色"),
    TextPlacement => ("text-placement", [], "Placement and text box", "位置与文本框"),
    TextAutomaticBoxHint => ("text-automatic-box-hint", [], "Zero uses the available canvas width and up to 160 pixels of height.", "设为 0 时，宽度使用画布剩余宽度，高度最多使用 160 像素。"),
    TextSaveChanges => ("text-save-changes", [], "Save text changes", "保存文字修改"),
    TextInsertTitle => ("text-insert-title", [], "Insert title frame", "插入标题帧"),
    TextAddSelected => ("text-add-selected", [], "Add text to selected frames", "向选中帧添加文字"),
    TextPreparing => ("text-preparing", [], "Preparing text…", "正在准备文字…"),
    TextCancelling => ("text-cancelling", [], "Cancelling text. Nothing will be added.", "正在取消文字操作，不会添加任何内容。"),
    TextCancelled => ("text-cancelled", [], "Text cancelled. The project is unchanged.", "文字操作已取消，项目未改变。"),
    TextAdded => ("text-added", [], "Text added to the selected frames. Its appearance is saved in the project.", "文字已添加到选中帧，其外观已保存在项目中。"),
    TextUpdated => ("text-updated", [], "Text updated. Timing and layer settings are unchanged.", "文字已更新，计时和图层设置保持不变。"),
    TextTitleInserted => ("text-title-inserted", [], "Title frame inserted and selected. Undo restores the original timeline.", "标题帧已插入并选中，撤销可恢复原来的时间轴。"),
    TextWorkerExited => ("text-worker-exited", [], "Text worker exited. You can retry.", "文字后台任务已退出，可以重试。"),
    TextTargetChanged => ("text-target-changed", [], "The project or frame selection changed while preparing text. Nothing was added; select the intended frames and retry.", "准备文字期间，项目或选中帧已改变。未添加任何内容，请重新选择目标帧后重试。"),
    TextAlreadyPreparing => ("text-already-preparing", [], "Text is already being prepared.", "文字正在准备中。"),
    TextLoadedTargetChanged => ("text-loaded-target-changed", [], "The project or selection changed since loading this text. Choose the saved text again before editing.", "载入这段文字后，项目或选区已改变。请重新选择已保存的文字后再编辑。"),
    TextTitleNeedsCurrent => ("text-title-needs-current", [], "Select a frame to insert a title after it, or choose At start.", "请先选择一帧以在其后插入标题，或选择“在开头”。"),
    TextTitleDurationRequired => ("text-title-duration-required", [], "Title duration must be positive and fit the project clock.", "标题时长必须大于零，且不能超出项目时钟范围。"),
    TextWorkerStartFailed => ("text-worker-start-failed", ["error"], "Could not start text worker: {error}", "无法启动文字后台任务：{error}"),
    TextStartFailed => ("text-start-failed", ["error"], "Could not prepare text: {error}", "无法准备文字：{error}"),
    TextSaveFailed => ("text-save-failed", ["error"], "Could not save text: {error}", "无法保存文字：{error}"),
    TextLoadFailed => ("text-load-failed", ["error"], "Could not load saved text: {error}", "无法载入已保存的文字：{error}"),
    TextEditSaved => ("text-edit-saved", [], "Edit saved text…", "编辑已保存的文字…"),
    TextNew => ("text-new", [], "New text", "新建文字"),
    TextAsTitle => ("text-as-title", [], "Insert as a title frame", "作为标题帧插入"),
    TextAtStart => ("text-at-start", [], "At start", "在开头"),
    TextAfterCurrent => ("text-after-current", [], "After current frame", "在当前帧之后"),
    TextTitlePosition => ("text-title-position", [], "Title position", "标题位置"),
    TextPositionInside => ("text-position-inside", [], "The text position must be inside the canvas.", "文字位置必须位于画布内。"),
    TextBoxFits => ("text-box-fits", [], "The text box must fit inside the canvas.", "文本框必须完全位于画布内。"),
    TextEmpty => ("text-empty", [], "Enter some text first.", "请先输入文字。"),
    TextTooLong => ("text-too-long", ["maximum"], "Text must be at most {maximum} UTF-8 bytes.", "文字最多可包含 {maximum} 个 UTF-8 字节。"),
    TextFontSizeRange => ("text-font-size-range", ["minimum", "maximum"], "Font size must be between {minimum} and {maximum} pixels.", "字号必须在 {minimum} 到 {maximum} 像素之间。"),
    TextFontFamilyLength => ("text-font-family-length", ["maximum"], "Enter a font family of at most {maximum} bytes.", "请输入字体族名称，长度不能超过 {maximum} 字节。"),
    TextFontMissing => ("text-font-missing", [], "The selected font family is not installed. Choose sans-serif or an installed family.", "所选字体族未安装，请选择 sans-serif 或已安装的字体族。"),
    TextGlyphBudget => ("text-glyph-budget", [], "This text needs too much rasterization memory. Use less text or a smaller font size.", "这段文字所需的栅格化内存过多，请减少文字或缩小字号。"),
    TextDimensions => ("text-dimensions", ["minimum", "maximum"], "The text box must be between {minimum} and {maximum} pixels in each dimension.", "文本框的宽度和高度都必须在 {minimum} 到 {maximum} 像素之间。"),
    TextInvisible => ("text-invisible", [], "Text color must not be fully transparent.", "文字颜色不能完全透明。"),
    TextMissingGlyphs => ("text-missing-glyphs", [], "Some characters have no installed font. Install a font for this script and try again.", "已安装的字体无法显示部分字符，请安装支持相应文字的字体后重试。"),
    TextDoesNotFit => ("text-does-not-fit", [], "The text does not fit. Increase the box size or reduce the font size.", "文字无法放入文本框，请增大文本框或缩小字号。"),
    TextAllocation => ("text-allocation", [], "Could not allocate the text image.", "无法为文字图像分配内存。"),
    TextSavedFrame => ("text-saved-frame", ["layer", "name", "count"], "Layer {layer} · {name} · {count} frame", "图层 {layer} · {name} · {count} 帧"),
    TextSavedFrames => ("text-saved-frames", ["layer", "name", "count"], "Layer {layer} · {name} · {count} frames", "图层 {layer} · {name} · {count} 帧"),
    TextSavedItem => ("text-saved-item", ["layer", "name", "count"], "Layer {layer} · {name} · {count} timed item", "图层 {layer} · {name} · {count} 个时间定位项"),
    TextSavedItems => ("text-saved-items", ["layer", "name", "count"], "Layer {layer} · {name} · {count} timed items", "图层 {layer} · {name} · {count} 个时间定位项"),
