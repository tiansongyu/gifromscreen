...
impl ProjectManifest {
    ...
    pub fn validate(&self) -> Result<(), DomainError> {
        ...
        let frame_positions: BTreeMap<_, _> = self
            ...
            .iter()
            .enumerate()
            .map(|(index, frame)| (frame.id, (index, frame.duration)))
            .collect();
        ...
        for transition in &self.timeline.transitions {
            ...
            let Some((from_index, from_duration)) = frame_positions.get(&transition.from_frame)
            else {
            ...
            let Some((to_index, to_duration)) = frame_positions.get(&transition.to_frame) else {
                issues.push(ValidationIssue::TransitionFrameMissing {
                    transition_id: transition.id,
            ...
            if transition.duration > *from_duration || transition.duration > *to_duration {
                issues.push(ValidationIssue::TransitionTooLong {
                    transition_id: transition.id,
                });
            }
        }
