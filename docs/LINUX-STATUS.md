...
- [x] Ordinary drawing press ownership, fast-batch input ordering and mapping-change cancellation; [scope and resource contract](DRAWING-PREVIEW-INPUT.md). These main-branch changes are newer than Preview 2 and do not establish full ink fidelity.
...
- [x] Ready-only X11 region-to-window snapping with validated WM/client-shadow hints, separate client/native-bound modes, bounded cancellable reads and stale-result rejection; [native geometry/GIF evidence](WINDOW-SNAP-QA-2026-09-08.md).
- [x] Click-armed X11 crosshair picker with passive target highlighting, temporary controller hiding, right/Escape cancellation and close-after-cleanup; [native selection/record/GIF evidence](X11-WINDOW-PICKER-QA-2026-09-08.md).
- [x] X11 press-on-button/drag-out/release activation with an owned InputOnly child, same-connection pointer-grab handoff, claimed/active separation, release-target identity and stale-layout replay; all twelve private-Xvfb tests pass on Rust 1.98.0 and 1.88.0. Click-to-pick remains available. See [implementation and bounded test evidence](X11-DRAG-PICKER-QA-2026-09-08.md).
- [x] Separate private GNOME drag acceptance: held/fast gestures, cancellation/cleanup, frame/client bounds, 30-frame recording, fixed static-pixel comparison and reopened GIF export. Child-window parity, partial-window policy and wider hardware/WM/CSD/mixed-DPI acceptance remain open.
...
- [x] Pass bounded [native Cinemagraph pen/erasers/selection/Undo/reopen/GIF acceptance](NATIVE-CINEMAGRAPH-QA-2026-09-07.md), correcting the actual justified-layout image-coordinate defect.
- [ ] Complete exact upstream ink/erase/Boolean geometry fidelity and broader Cinemagraph input/platform coverage, distinct from Rectangular freeze and typed-snapshot composition.
...
- [x] Persisted System/manual language choices and initial English/Chinese launcher, recorder and primary-editor controls; [current preview/crop/export extension](PREVIEW-EXPORT-LOCALIZATION.md).
- [ ] Complete Chinese and English localization of all application-owned UI, followed by the remaining 27 target catalogs and text-input/rendering acceptance; [coverage plan](LOCALIZATION-PLAN.md).
