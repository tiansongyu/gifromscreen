...
| 选区拖动、尺寸输入、窗口吸附 | M2 | 完整 | 受限 | 完整 | Wayland 无通用窗口几何接口，吸附只在可得信息范围内工作 |
| 可调 FPS | M1 | 完整 | 完整 | 完整 | 内部按单调时钟采样，不依赖显示刷新率 |
| 固定帧率 | M1 | 完整 | 完整 | 完整 | 丢帧时补偿时长，不伪造未捕获画面 |
| 仅捕获变化 | M2 | 完整 | 完整 | 完整 | damage 元数据优先，像素差分回退 |
| 手动快照模式 | M2 | 完整 | 完整 | 完整 | 每次触发增加一帧 |
| 交互触发模式 | M3 | 完整 | 受限 | 完整 | Wayland 无通用被动全局输入事件 |
| 每秒/分钟/小时频率 | M2 | 完整 | 完整 | 完整 | 对齐上游 CaptureFrequencies |
| 倒计时 | M1 | 完整 | 完整 | 完整 | 可取消 |
