...
2026-09-09 发布收敛：本表保留设计目标，**不作为已完成清单**。
v0.1.0 的实际已交付、已知限制和暂缓任务以[工作总结](WORK-STATUS.md)为准；
下载与说明见[正式版记录](releases/v0.1.0.md)。当前不继续扩展功能。
...
| 自由绘制 | M2 | 画笔/高亮/橡皮/选择 |
| 形状 | M2 | 矩形、椭圆、三角形、封闭块状箭头；小数描边、填充、圆角与直接操纵。保留已有线和线段箭头扩展；虚线不是固定上游已接通的 GUI 基线。实现及未通过的像素对照见[验收记录](VECTOR-SHAPE-QA-2026-09-09.md)。 |
