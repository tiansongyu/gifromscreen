...
Linux 已接入[有序整帧编辑](ORDERED-FRAME-EDITING.md)：先添加的标注会随之后的裁剪、缩放、旋转、翻转和现有效果一起变化；后添加的标注保持在后续阶段。原生缩放／旋转、旧层转换和 GIF 一致性见[验收记录](ORDERED-GEOMETRY-QA-2026-09-07.md)。扩画布阴影、混合内外边框、隐藏中间阶段烘焙及 WPF 数值像素对齐仍未完成，不能将基础操作已接通等同于完全一比一。
