...
- [产品与软件架构设计](DESIGN.md)
- [ScreenToGif 功能对照表](FEATURE_MATRIX.md)
- [ADR-0001：Rust 桌面技术栈](ADR-0001-RUST-DESKTOP-STACK.md)
- [Linux 实现状态](LINUX-STATUS.md)
- [Linux 性能证据](LINUX-BENCHMARKS.md)
- [实际功能差距审计](PARITY-AUDIT.md)
- [视频导入、工程插入与转场验收](LINUX-ITERATION-VIDEO.md)
- [摄像头、画板、高级编辑与最新分发包验收](LINUX-ITERATION-LIVE-SOURCES.md)
- [输入标注、自动任务与循环匹配验收](LINUX-ITERATION-ANNOTATIONS.md)
- [进度与输入标注](ANNOTATIONS.md)
- [自动编辑任务预设](AUTOMATIC-TASKS.md)
- [标注作者范围与重编辑](ANNOTATION-AUTHORING-SCOPE.md)
- [烘焙后的输入数据保护与旧工程确认](CAPTURE-INPUT-BINDING.md)
- [隔离 GNOME / Portal / PipeWire 实测记录](WAYLAND-NESTED-QA-RESULTS-2026-09-06.md)
- [Linux 预览包安装与构建](PACKAGING.md)
- [Cinemagraph 笔迹编辑与原生验收](NATIVE-CINEMAGRAPH-QA-2026-09-07.md)
- [独立 WPF 数值对照与剩余差异](INK-REFERENCE-EVIDENCE.md)
- [下一轮录制控制与精确定位计划](NEXT-LINUX-ITERATION.md)
- [全局录制快捷键](GLOBAL-SHORTCUTS.md) 与 [X11 录制框原生验收](X11-RECORDER-WINDOWS.md)
...
- X11 录制采用独立原生边框和控制面板，物理选区不再受工具条大小影响；面板优先放到选区外，全屏无空位时隐藏／最小化，恢复窗口后先暂停再显示控制。Wayland 使用独立源内裁剪控制器。[原生验收与剩余限制](NATIVE-X11-SPLIT-QA-2026-09-07.md)
...
- X11 显示器录制可在开始前展开“Fit region to a window…”吸附窗口：默认校验窗框提示，也可选择客户区或原生外框。框内可刷新新开／改名窗口，不改已有选区；点击吸附时读取新位置，取消／选区改变会使旧结果失效，越界不会静默缩小。这是一次性定位，不是窗口跟踪。[原生验收与边界](WINDOW-SNAP-QA-2026-09-08.md)
- 同一面板的“Pick window on screen…”提供十字准星选窗：从按钮按住拖到目标后松开确认，也可先点击进入再选目标。原生抓取成功后暂时隐藏控制，右键／Escape 取消，清理结束后才恢复。新增拖选通过 12 项双 Rust 版本私有 Xvfb 回归，并在隔离 GNOME 验证极速拖放、取消、录制及 GIF 导出；子窗口、部分窗口及硬件／混合 DPI 验收仍待补齐。[拖选实现与验收](X11-DRAG-PICKER-QA-2026-09-08.md)、[原点击选窗验收](X11-WINDOW-PICKER-QA-2026-09-08.md)
...
- X11 可选择桌面交互快照：按键按下、鼠标按下和滚轮触发，默认每帧播放 500 ms；暂停清掉待处理输入，无输入也能定时结束。默认不保存按键／点击事件内容。当前会包含录制器自身操作，不提供精确控件排除或触发延时；Wayland 不提供此模式。[实现边界与原生验收](X11-INTERACTION-SAMPLING.md)
- 首帧到达后即流式写入内容寻址素材和同步 journal；异常失败会保留可恢复项目路径，停止后再完成 checkpoint。
- 停止后自动创建同名 `.gfsproj`，进入带虚拟胶片条、逐帧预览、选择、排序、删除、延时和 undo/redo 的编辑器。
- 编辑器支持 Fit、100%／200% 原像素预览和滚动／中键平移；可直接拖出裁剪草稿并同步编辑像素边界，显式应用到整段动画后支持 Undo/Redo。裁剪、尺寸变换、90° 旋转和翻转作用于完整合成画面，写入可恢复历史。[边界与原生验收](EDITOR-CANVAS-QA-2026-09-08.md)
