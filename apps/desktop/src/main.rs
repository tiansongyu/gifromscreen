...
struct RecordingJob {
    shortcut_state: recorder_shortcuts::LiveShortcutState,
...
impl eframe::App for GifFromScreenApp {
    fn update(&mut self, context: &egui::Context, frame: &mut eframe::Frame) {
        ...
        self.poll_recorder_shortcuts(context);
...
impl GifFromScreenApp {
    ...
    fn restore_main_window_if_requested(&mut self, context: &egui::Context) {
            ...
            .filter(|snapshot| matches!(snapshot.restore, MainWindowRestore::X11Geometry))
        {
            ...
            context.send_viewport_cmd(egui::ViewportCommand::MinInnerSize(
                egui::vec2(680.0, 440.0).min(snapshot.size) / scale,
            ));
            context.send_viewport_cmd(egui::ViewportCommand::InnerSize(snapshot.size / scale));
            ...
            if let Some(position) = snapshot.position {
                context.send_viewport_cmd(egui::ViewportCommand::OuterPosition(position / scale));
            }
        }
    ...
    fn show_screen_recorder(&mut self, ui: &mut egui::Ui) {
        ...
        egui::ScrollArea::vertical()
            ...
            .show(ui, |ui| self.show_screen_recorder_options(ui));
    }
    ...
    fn recorder_stage(&self) -> RecorderStage {
        if self
            .recorder_overlay
            .as_ref()
            .is_some_and(|overlay| overlay.pending_live_start.is_some())
        {
            return RecorderStage::Countdown(0);
        }
    ...
    fn prepare_live_recording(&mut self) -> Result<(), String> {
        if let Some(overlay) = &mut self.recorder_overlay {
            overlay.pending_live_start = Some(Instant::now());
            Ok(())
        } else {
...
mod tests {
    ...
    fn paused_indicator_waits_for_capture_acknowledgement_and_rejects_stale_messages() {
        ...
        let mut job = super::RecordingJob {
            shortcut_state: super::recorder_shortcuts::LiveShortcutState::default(),
            receiver,
            controller,
